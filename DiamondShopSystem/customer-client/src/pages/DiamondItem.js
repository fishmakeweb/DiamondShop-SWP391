import React, { useEffect, useState, useContext } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';
import RecommendItem from '../components/RecommendItem';
import axios from '../axios.js';
import CartContext from '../components/CartContext.js';

const Item = () => {
  let navigate = useNavigate();
  return (
    <>
      <button onClick={() => navigate(-1)}>BACK TO SELECTION</button>
    </>
  );
};

function DiamondDetails({ diamond }) {
  return (
    <div className="p-4 bg-white shadow rounded-lg">
      <table className="min-w-full">
        <tbody>
          {diamond.price && (
            <tr>
              <th>Price:</th>
              <td>${diamond.price}</td>
            </tr>
          )}
          {diamond.shape && (
            <tr>
              <th>Shape:</th>
              <td>{diamond.shape.shapeId} - {diamond.shape.shapeDescription}</td>
            </tr>
          )}
          {diamond.carat && (
            <tr>
              <th>Carat:</th>
              <td>{diamond.carat.carat}</td>
            </tr>
          )}
          {diamond.color && (
            <tr>
              <th>Color:</th>
              <td>{diamond.color.colorDescription}</td>
            </tr>
          )}
          {diamond.clarity && (
            <tr>
              <th>Clarity:</th>
              <td>{diamond.clarity.clarityDescription}</td>
            </tr>
          )}
          {diamond.cut && (
            <tr>
              <th>Cut:</th>
              <td>{diamond.cut.cutDescription}</td>
            </tr>
          )}
          {diamond.fluorescence && (
            <tr>
              <th>Fluorescence:</th>
              <td>{diamond.fluorescence.fluorescenceDescription}</td>
            </tr>
          )}
          {diamond.gia && (
            <tr>
              <th>GIA Issue Date:</th>
              <td>{new Date(diamond.gia.issueDate).toLocaleDateString()}</td>
            </tr>
          )}
          {diamond.measurement && (
            <tr>
              <th>Measurement:</th>
              <td>{`Length: ${diamond.measurement.length}, Width: ${diamond.measurement.width}, Height: ${diamond.measurement.height}`}</td>
            </tr>
          )}
          {diamond.polish && (
            <tr>
              <th>Polish:</th>
              <td>{diamond.polish.polishDescription}</td>
            </tr>
          )}
          {diamond.symmetry && (
            <tr>
              <th>Symmetry:</th>
              <td>{diamond.symmetry.symmetryDescription}</td>
            </tr>
          )}
        </tbody>
      </table>
    </div>
  );
}

function DiamondItem() {
  const { diamondId } = useParams();
  const [itemDetails, setItemDetails] = useState(null);
  const { addToCart } = useContext(CartContext);
  const [buttonText, setButtonText] = useState('ADD TO BAG');
  const [showDetails, setShowDetails] = useState(false);

  const handleClick = () => {
    setButtonText('ADDED');

    setTimeout(() => {
      setButtonText('ADD TO BAG');
    }, 1500);
  };

  useEffect(() => {
    console.log(`Fetching diamond details for ID: ${diamondId}`);
    axios.get(`/diamonds/${diamondId}`)
      .then(response => {
        console.log('Fetched diamond details:', response.data);
        setItemDetails(response.data);
      })
      .catch(error => {
        console.error('Error fetching diamond details:', error);
      });
  }, [diamondId]);

  if (!itemDetails) return <div>Loading...</div>;

  return (
    <div className="flex flex-col bg-white">
      <div className="h-24 w-full"></div>
      <Navbar />
      <div className="self-center mt-20 w-full max-w-[1214px] max-md:mt-10 max-md:max-w-full">
        <div className="flex gap-5 max-md:flex-col max-md:gap-0">
          <div className="flex flex-col w-2/5 max-md:ml-0 max-md:w-full">
            <img
              loading="lazy"
              src={`/img/diamonds/${itemDetails.img}`}
              alt={itemDetails.name}
              className="mt-20 w-80 h-80 object-cover mx-auto"
            />
          </div>
          <div className="flex flex-col ml-5 w-3/5 max-md:ml-0 max-md:w-full">
            <div className="flex flex-col px-5 mt-10 max-md:mt-10 max-md:max-w-full">
              <div className="flex gap-5 justify-between max-md:flex-wrap max-md:max-w-full">
                <div className="flex flex-col">
                  <div className="text-base hover:text-custom-brown text-neutral-600"><Item /></div>
                  <div className="mt-11 text-4xl text-black max-md:mt-10">
                    {itemDetails.name}
                  </div>
                  <div className="mt-4 text-2xl font-bold text-black text-opacity-50">
                    ${itemDetails.price}
                  </div>
                </div>
                <img
                  loading="lazy"
                  alt=""
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/993591a3a81dab2d2c2ebb7f3b4a931dab5d810de16c88f48d4f1c172185adfc?"
                  className="shrink-0 my-auto aspect-[0.94] w-[35px]"
                />
              </div>
              <div className="mt-9 text-xl text-black font-[350] max-md:max-w-full">
                Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
                eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut
                enim ad minim veniam, quis nostrud exercitation ullamco laboris
                nisi ut aliquip ex ea commodo consequat.
                <br />
              </div>
              <button
                className="justify-center w-36 self-start px-4 py-4 mt-9 text-lg border border-solid rounded-full bg-black text-white hover:bg-opacity-80 max-md:mx-auto max-md:max-w-full max-md:py-2"
                onClick={() => { handleClick(); addToCart(itemDetails); }}
              >
                {buttonText}
              </button>
            </div>
            <button
              className="flex gap-5 hover:text-custom-brown justify-center px-0.5 py-3 mt-9 text-xl whitespace-nowrap border-b border-solid border-zinc-400 text-neutral-700 max-md:flex-wrap max-md:max-w-full"
              onClick={() => setShowDetails(!showDetails)}
            >
              DETAILS
              <img
                loading="lazy"
                alt=""
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/8e79619d6d6b36f08d933bffaf65933ca66414fb4b6e9b0f6c0ed83858787e4f?"
                className="shrink-0 w-6 aspect-square"
              />
            </button>
            {showDetails && <DiamondDetails diamond={itemDetails} />}
          </div>
        </div>
      </div>
      <RecommendItem />
      <Footer />
    </div>
  );
}

export default DiamondItem;
